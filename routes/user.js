...
router.get("/checkUserExist/:mobile", async (req, res) => {
  ...
  try {
    if (!mobile) {
      ...
      const getData = await getMultipleItemsByQuery(
        TABLE_NAME,
        indexName,
  ...
  try {
    if (!body.mobile) {
      ...
      const userParams = {
        TableName: "users",
        FilterExpression:
          "isSocialLogin = :socialLogin AND mobile = :mobileData",
        ExpressionAttributeValues: {
      ...
      if (firstTimeuser.length > 0) {
        ...
        if (!body.fullName) {
          ...
          let image = "";
          const isMember =
            body.isMember == "true" || body.isMember == "false"
              ? body.isMember
              : false;
          const item = {
            id: body.id,
            ...
            dateOfJoining: body.dateOfJoining
              ? new Date(body.dateOfJoining).toISOString()
              : "",
            image: image,
            isMember: isMember,
            isSocialLogin: true,
      ...
      if (firstTimeuser.length > 0) {
        ...
        if (body.mobile == "9876543210") {
          ...
          const response = await axios.get(
            `https://2factor.in/API/V1/${process.env.SMS_KEY}/SMS/+91${body.mobile}/${otp}/OTP1`
          );
  ...
  try {
    if (!body.mobile) {
      ...
      if (getData.Items.length > 0) {
        ...
        const filterData = await filterItemsByQuery(
          TABLE_NAME,
          KeyConditionExpression,
          ExpressionAttributeValues,
          FilterExpression
      ...
      if (body.sessionId == "d8039ce8-3088-41f1-8e08-10bd3b99ce1e") {
        ...
        if (getData.Items.length > 0 && body.otp == "9231") {
          ...
          res.success({
            ...
            data: { message: "OTP Matched", data: resp },
          });
        } else {
        ...
        const response = await axios.get(
          `https://2factor.in/API/V1/${process.env.SMS_KEY}/SMS/VERIFY/${body.sessionId}/${body.otp}`
        );
        console.log(response.data);
        const resp = response.data;
        if (resp.Status === "Success") {
...
router.post("/cities", async (req, res) => {
  ...
  try {
    if (!body.name) {
      ...
      const isunique = cities.Items.find(
...
router.post("/districts", async (req, res) => {
  ...
  try {
    if (!body.name) {
      ...
      const isunique = districts.Items.find(
        (district) => district.name.toLowerCase() === body.name.toLowerCase()
      );
