...
router.get("/", async (req, res) => {
  try {
    ...
    const restrictedNews =
      items.Items.length > 0
        ? items.Items.filter(
            (val) => val.isVisible === true || val.isVisible === "true"
          )
        : [];
...
router.put("/:id", verifyToken, upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (findNews.Item) {
      ...
      if (body.like == "true") {
        like = data.like.includes(req.user.id)
          ? [...data.like]
          : [...data.like, req.user.id];
      } else if (body.like == "false") {
        like =
          data.like.length > 0
            ? data.like.filter((id) => id !== req.user.id)
            : [];
      }
      ...
      if (body.share) {
        share = data.share.includes(req.user.id)
          ? [...data.share]
          : [...data.share, req.user.id];
      }
      const toggle =
        body.toggle == 1 || body.toggle == 0 ? body.toggle : data.toggle;
      const isVisible =
        body.isVisible === "true" || body.isVisible === true
          ? true
          : body.isVisible === "false" || body.isVisible === false
          ? false
          : data.isVisible;
